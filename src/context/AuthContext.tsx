...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const refreshToken = async (): Promise<boolean> => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL_LOCAL_LOCAL}/api/auth/refresh`,
  ...
  const logout = async () => {
    try {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_API_URL_LOCAL_LOCAL}/api/auth/logout`, {
        method: 'POST',
        credentials: 'include',
      });
    } catch (err) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL_LOCAL_LOCAL}/api/auth/me`, {
          credentials: 'include',
        });
        ...
        if (res.status === 401) {
          const refreshed = await refreshToken();
          if (refreshed) {
            const retry = await fetch(`${process.env.NEXT_PUBLIC_API_URL_LOCAL_LOCAL}/api/auth/me`, {
              credentials: 'include',
            });
            if (retry.ok) {
